...
const CustomTab = lazy(
  () => import("../../../../shared/components/Tabs/Tab/index.js"),
);
...
const ForexTable = ({ accountData }) => {
  ...
  const calculateTotalProfit = (interval) => {
    ...
    return filteredData.reduce(
      (sum, dataPoint) => sum + (dataPoint?.profit || 0),
      0,
    );
  };
  ...
  return (
    <TabContext value={value}>
      <Box>
        <TabList>
          <CustomTab
            label={`24 ${t("forex.settings.table.tab.hour")}`}
            value="1"
            selected={selected}
            handleChange={handleChange}
          />
          <CustomTab
            label={`7 ${t("forex.settings.table.tab.day")}`}
            value="2"
            selected={selected}
            handleChange={handleChange}
          />
          <CustomTab
            label={`30 ${t("forex.settings.table.tab.day")}`}
            value="3"
            selected={selected}
            handleChange={handleChange}
          />
