...
const ForexTabPanel = ({ value, label, accountData }) => {
      ...
      renderFunction = () =>
        renderTable(
          ...
          (currentDate, timestamp) =>
            currentDate - timestamp * 1000 < 24 * 60 * 60 * 1000,
        );
    ...
    case "2":
      renderFunction = () =>
        renderTable(
          ...
          currentDate,
          (currentDate, timestamp) =>
            currentDate - timestamp * 1000 < 7 * 24 * 60 * 60 * 1000,
        );
      itemCount = renderFunction().length;
      break;
    case "3":
      renderFunction = () =>
        renderTable(
          ...
          currentDate,
          (currentDate, timestamp) =>
            currentDate - timestamp * 1000 < 30 * 24 * 60 * 60 * 1000,
        );
      itemCount = renderFunction().length;
      break;
    default:
  ...
  return (
    ...
    >
      ...
      >
        {({ index, style }) => (
          <div style={style}>{renderFunction()[index]}</div>
        )}
      </FixedSizeList>
    </TabPanel>
  );
...
const renderTable = (accountData, currentDate, filterFunction) => {
  ...
  return filteredTimestamps.map((timestamp) => {
    const dataPoint = accountData?.data?.statistics?.find(
