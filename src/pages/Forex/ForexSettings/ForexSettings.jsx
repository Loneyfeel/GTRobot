...
const ForexSettings = ({
  ...
  const fetchAccountData = async () => {
    try {
      setIsLoading(true);
      const response = await axios.post(`${host}/api/get-forex-data`, {
        initData,
        userId,
      });
      if (
  ...
  const [tradeIsEnabled, setTradeIsEnabled] = useState(
    accountData?.data?.status || false,
  );
  ...
  const handleToggle = async () => {
    try {
      setIsLoading(true);
      const response = await axios.post(`${host}/api/update-forex-status`, {
        initData,
        userId,
  ...
  const handleLogout = async () => {
    try {
      setIsLoading(true);
      const response = await axios.post(`${host}/api/delete-forex-data`, {
        initData,
        userId,
      });
      if (response.data.errorCode) {
