...
const TaskList = ({ activeTasks, setActiveTasks }) => {
  ...
  const handleTaskClick = async (taskId, task_group_id) => {
    ...
    const taskClickCounts =
      JSON.parse(localStorage.getItem(taskClickCountKey)) || {};
    ...
    window.open(
      tasks.find((task) => task.task_id === taskId).task_link,
      "_blank",
    );
  ...
  const handleButtonClick = async () => {
    ...
    try {
        ...
        if (taskClickCounts[task.task_id] >= 2 && task.is_required === 1) {
          ...
          if (response.status === 200) {
            ...
            localStorage.setItem(
              taskClickCountKey,
              JSON.stringify(taskClickCounts),
            );
          } else {
            console.error(
              `Ошибка при сохранении задания ${task.task_id}: ${response.status}`,
            );
      ...
      console.error(
        "Ошибка при обработке нажатия на кнопку проверки заданий:",
        error,
      );
    }
    console.log(localStorage);
              ...
              >
                ...
                >
                  ...
                  <Typography variant="h6">
                    {translateText(tasks[index].task_text)}
                  </Typography>
                </CardContent>
              </Card>
            )}
